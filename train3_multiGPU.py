...
# Constants
CHECKPOINT_DIR = "./whisper_farsi_checkpoints_all_3andmusic_multiGPU"
...
class StreamingAudioDataset(TorchIterableDataset):
    def __init__(self, csv_path, chunk_size=32, processor=None):
        ...
        self.start_idx = 0
        self.end_idx = None  # Will be set by worker_init_fn
        
    def __iter__(self):
        chunk_buffer = []
        
        # Read only this worker's portion of the data
        df_iterator = pd.read_csv(
            self.csv_path,
            skiprows=self.start_idx,
            nrows=self.end_idx - self.start_idx if self.end_idx else None,
...
def worker_init_fn(worker_id):
    ...
    # Read the full CSV to get total size
    df = pd.read_csv(dataset.csv_path)
    num_samples = len(df)
    
    # Calculate the chunk size for each worker
    per_worker = int(np.ceil(num_samples / float(num_workers)))
    
    # Calculate start and end indices for this worker
    worker_start = worker_id * per_worker
    worker_end = min(worker_start + per_worker, num_samples)
    
    # Update the dataset's chunk boundaries for this worker
    dataset.start_idx = worker_start
    dataset.end_idx = worker_end
...
def main():
    ...
    train_dataset = StreamingAudioDataset(
        "./new_cleaned_manifests_with_music/train_combined2.csv",
        chunk_size=32,
        processor=processor
    )

    eval_dataset = StreamingAudioDataset(
        "./new_cleaned_manifests_with_music/dev_combined2.csv",
        chunk_size=32,
        processor=processor
    )
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        learning_rate=1e-5,
        warmup_steps=500,
        max_steps=22000,  # Adjust this for daily training length
    ...
    trainer.get_train_dataloader()._worker_init_fn = worker_init_fn
    trainer.get_eval_dataloader()._worker_init_fn = worker_init_fn
    ...
    new_manifest_path = "./new_cleaned_manifests_with_music/test_combined2.csv"
...
if __name__ == "__main__":
    ...
    # python3 -m torch.distributed.launch --nproc_per_node=8 train3_multiGPU.py
